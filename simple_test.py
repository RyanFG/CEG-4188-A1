...
class SimpleTest():
    ...
    def setup(self, port, host="localhost"):
        ...
        self.alice_client = Popen(["python", "client.py", "Alice", host, str(port)], stdin=PIPE, stdout=PIPE, text=True)
        self.kay_client = Popen(["python", "client.py", "Kay", host, str(port)], stdin=PIPE, stdout=PIPE, text=True)
        time.sleep(SLEEP_SECONDS)
